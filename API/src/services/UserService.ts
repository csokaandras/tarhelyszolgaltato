...
export const registerUser = async (name: string, email: string, password: string, phone?: string, address?: string) => {
    ...
    const user = userRepository.create({ name, email, phone, address, password: hashedPassword });
...
export const getAllUsers = async () => {
    return await userRepository.find({ select: ["id", "name", "email", "phone", "address"] });
};
 
export const getUserById = async (id: string) => {
    return await userRepository.findOne({ where: { id }, select: ["id", "name", "email", "phone", "address"] });
};
